...
<div class="container">
  <div class="row">
    <div class="col-md-12">
      <a href="crear_cliente.php"  class="btn btn-primary mt-4">Nuevo Registro</a>
      <hr>
...
if (isset($_POST['papellido'])) {
  $consultaSQL = "SELECT * FROM cliente WHERE primerApellido LIKE '%" . $_POST['papellido'] . "%'";
} else {
  $consultaSQL = "SELECT * FROM cliente";
}
?>
        ...
        <tbody>
          ...
          if ($registro && $sentencia->rowCount() > 0) {
            foreach ($registro as $fila) {
              ?>
              <tr>
                <td><?php echo escapar($fila["idCliente"]); ?></td>
                ...
                <td>
                  <a href="<?= 'eliminar_cliente.php?documento=' . escapar($fila["idCliente"]) ?>" . >🗑️Eliminar</a>
                  <a href="<?= 'actualizar_cliente.php?documento=' . escapar($fila["idCliente"]) ?>" . >✏️Actualizar</a>
                </td>
              </tr>
              <?php
            }
